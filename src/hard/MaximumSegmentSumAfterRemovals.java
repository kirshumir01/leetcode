...
public class MaximumSegmentSumAfterRemovals {
    public long[] maximumSegmentSum(int[] nums, int[] removeQueries) {
        ...
        boolean[] isOccupied = new boolean[removeQueries.length];
        ...
        for (int i = removeQueries.length - 1; i >= 0; i--) {
            ...
            if ((index < nums.length - 1 && isOccupied[index + 1])) {
                rightEnd[index] = rightEnd[index + 1];
            }

            leftEnd[index] = index;
            if (index > 0 && isOccupied[index - 1]) {
                leftEnd[index] = leftEnd[index - 1];
            }

            int leftIndex = leftEnd[index];
            int rightIndex = rightEnd[index];
            long currentSegmentSum = prefixSum[rightIndex + 1] - prefixSum[leftIndex];
            currentMax = Math.max(currentMax, currentSegmentSum);
